...
"""
    java, pandas, numpy, pdfminer.six, tabula-py, PyPDF2, datetime
    __author__ = 'Andy Xu'
...
# from header_extracr_v01 import Module_name_list
...
input_RM_name = 'iMX8M_Nano_RM_Rev_B'
pdf_name = input_RM_name + '.pdf'
NXP_pdf = open(pdf_name, 'rb')

RM_extract = 'RM-' + input_RM_name + '-extarct'
if os.path.isdir(RM_extract):
    ss = os.getcwd() + '/' + RM_extract
    shutil.rmtree(ss)
os.mkdir(RM_extract)
ss = os.getcwd() + '/' + RM_extract
...
if not doc.is_extractable:
    ...
    # with open('temporary.txt','a+',errors='ignore') as ff:
    ...
    for line in range(len(content_sum) - 2):
        ...
        if fix_value != '00':
            ...
            if line_tidy.match(fix_value):
                ...
                # 该目录下的子目录项与目录不是同一章
                    ...
                    while (not line_str.match(content_sum[n])) or content_sum[n] == '00':  # 前三个没有小数点
                        n += 1
                    c_c = content_sum[n - 1]
                    if (n - line > 1) and ('.' not in c_c) and (c_c != '00') and ('hapter' not in c_c):
                        content_sum[line] = fix_value + ' ' + c_c + content_sum[n]
                        content_sum[n - 1] = '00'
                        content_sum[n] = '00'
                    else:
            ...
            # 伪目录行，需要和下面的行相加
                ...
                cc = content_sum[n + 1]
                if line_tidy_r.match(line_n) or line_n == '00':
                    continue
        ...
        # print(content_sum[line])
    content_sum = list(filter(lambda x: x != '00' and len(x) != 0, content_sum))

    # with open('test_8M.txt','a+',errors='ignore') as ff:
    #     for i in content_sum:
    #         ff.write(i+'\n')
    ...
    module_list[module_sto[-1]][1] = str(
        min((int(module_list[module_sto[-1]][1]) + 3), pagecount))  # 最后一个模块的寄存器页多一页，防止漏页
            ...
            if sort_number:
                ...
                for g in range(len(df.index)):
                    ...
                    if module_R in ab:  # 序列为1， 比如ADC1
                        if module_j and module_base_jug:
                        ...
                        if ef != sort_number_n:
                            cd = df.iat[g, 0]
                            if ('_' in cd) and module_base_jug:
    ...
    """
    the following code only used to debug, you can enable them and typing the page range to debug
    """

    # module_sto.remove('Transceiver_Registers')
    # module_list['BLE_RF_Registers'][1] = '1384'
    # # del module_sto[10:15]
    # module_list['AIPS-Lite'] = ['294', '301']
    # ModueNameMismatch_list1 = ['FlexIO', 'FlexSPI_Controller', 'SAI',
    #                           'FlexRAM', 'WDOG3', 'OCOTP_CTRL', 'WDOG1-2', 'eFlexPWM']
    # ModueNameMismatch_list2 = ['FLEXIO', 'FLEXSPI', 'I2S', 'FLEXRAM', 'RTWDOG', 'OCOTP','WDOG','PWM']
    # ace = []
    # start_page_num = 1085
    # end_page_num = 1123
    ...
    for i in module_sto:
        ...
        columns_size_r = 0  # reserve the column size of the first register module
        for n in range(end - start + 1):  # 遍历一个模块的有效每一页
            page_judge = False  # 每一页寄存器的第一个与最后一个判断
            print('parsing page:', c)
            if page_par:
                    ...
                    if df:
                        ...
                        if (module_valid is False) and (4 < columns_size < 8):  # 出现的第一个寄存器列表
                            ...
                            # 第一行没有模块名的寄存器列表
                                ...
                                if len_df > 2:
                                    while (b < 3) and not (
                                            ('egister' in str(df.iat[b, 1])) and ('idth' in str(df.iat[b, 2]))):
                                        b += 1
                                    if b == len_df:
                                        continue
                                if ('egister' in str(df.iat[b, 1])) and ('idth' in str(df.iat[b, 2])):
                                    ...
                                    df = tidy_last_line(df)
                                    DF = DF.append(rename_columns(df))  # 列名重命名，以便和后面再出现字典表相加
                            elif module_ind and (columns_size_r == columns_size):  # 第二页还有寄存器表的情况
                                d = 0
                                while (d < 3) and not (
                                        ('egister' in str(df.iat[d, 1])) and ('idth' in str(df.iat[d, 2]))):
                                    d += 1
                                if d == len(df.index):
                                    continue
                        ...
                        # 分寄存器的开始,整理寄存器列表
                            ...
                            # if module_name in ModueNameMismatch_list1:
                            #     module_name = ModueNameMismatch_list2[ModueNameMismatch_list1.index(module_name)]
                        ...
                        if (module_valid is True) and ((0 < columns_size < 4) or (128 < v[2] < 130)):
                            ...
                            if columns_size == 1:
                                ...
                                if dff:
                                    ...
                                    bit_dict_judge = True
                                    continue
                            # elif columns_size == 3 and v[2] > 200:
                            #     continue
                            else:
                                lm = df.iat[0, 0]
                            register_jud = False
                                        ...
                                        for qr in df.values:
                                            ...
                                            if not page_rect_first:
                                                ...
                                                if bit_dict:
                                                    ...
                                                    elif (bit_dict[bc] == 'lost name') and (
                                                            key[:5] == 'miss ') and page_end_field_end:
                                                        # 删除上一页最后一个bit,并将其内容与本页第一个bit合并
                                                        RDF.drop([len(RDF.index) - 1], inplace=True)
                                                        key = bc
                                                        del bit_dict[bc]
                                                    elif ('31' in key) or (
                                                            (bc[:5] == 'miss ') and (key[:5] == 'miss ')):
                                                        bit_dict = {}
                                                        n += 1
