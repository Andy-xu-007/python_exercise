...
input_RM_name = 'imx1064RT_RM_revB'
...
if not doc.is_extractable:
    ...
    line_fragmentary = re.compile(r'^\d+(\.\d+)+\s*.{3,}?\D$')   # 没有目录点和页码的行

    # 目录整理
    CP_num = '提取章节序号'
    for line in range(len(content_sum)):
        fix_value = content_sum[line]
        if 'hapter' in fix_value:
            CP_num = fix_value.split(' ')[-1]
        n = line + 1
        # 出现纯数字行
        if line_tidy.match(str(fix_value)):
            if CP_num == fix_value.split('.')[0]:
                while content_sum[n] == '00' or not line_str.match(content_sum[n]):  # 数字 + 字母开头的目录行
                    n += 1
            elif int(fix_value.split('.')[0]) > int(CP_num):
                # 找到与当前序号对应的章节
                ...
                while (not line_str.match(content_sum[n])) or content_sum[n] == '00':  # 前三个没有小数点
                    n += 1
            content_sum[line] = str(fix_value) + ' ' + str(content_sum[n])
            content_sum[n] = '00'
        # 没有号的残缺目录行
        if line_str.match(str(fix_value)):
            fix_value_1 = content_sum[line-1]
            if line_fragmentary.match(fix_value_1):
                content_sum[line - 1] = str(fix_value_1) + ' ' + str(fix_value)
                content_sum[line] = '00'
            else:
                while not line_tidy.match(content_sum[n]):
                    n += 1
                content_sum[line] = str(content_sum[n]) + ' ' + str(fix_value)
                content_sum[n] = '00'
    ...
    for ii in module_sto:
        print(module_list[ii])
    ...
    def Register_name_filter(df, module_N):
        ...
        if module_R in first_value:
            ...
            first_V = re_module_filter.findall(first_value)[-1] # 提取括号内的内容
            sort_number = module_number.findall(first_V)  # 提取第一个序列，是字符
            ef = sort_number
            # 寄存器列表有多个子集
            if sort_number:
                ...
                for g in range(len(df.index)):
                    ...
                    if module_R in ab:  # 序列为1， 比如ADC1
                        ...
                        ab = ab.replace(module_R,'',1)  # 还原寄存器名
                        df.iat[g, 1] = register_name_n(ab)
                        # df_new[g] = df.iat[g]
                        df_new = df_new.append(df.loc[[g]],ignore_index=True)    # 行相加，目前效率较低
                    else:  # 序列为1之后的值，比如ADC2,3,4...
                        first_V = re_module_filter.findall(ab)[-1]  # 提取括号内容
                        sort_number_n = module_number.findall(first_V)  # 提取第一个序列
                        if ef != sort_number_n:
                            cd = df.iat[g, 0]
                            if '_' in cd:
                                            ...
                                            for qr in df.values:
                                                ...
                                                if not page_rect_first:
                                                    ...
                                                    if bit_dict:
                                                        ...
                                                        elif page_end_field_end and page_start_field_start:
